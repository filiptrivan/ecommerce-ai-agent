...
import { ExternalProduct, Notification, NotificationSaveBody, QdrantProduct, Agent, User, UserNotification, AgentSaveBody, UserSaveBody, UserNotificationSaveBody } from '../../entities/business-entities.generated';

@Component({
    selector: 'agent-base-details',
    template: `
        ...
        <panel-body>
            @defer (when loading === false) {
                <form class="grid">
                    ...
                    <div *ngIf="showTemperatureForAgent" class="col-12 md:col-6">
                        <spiderly-number [control]="control('temperature', agentFormGroup)" [decimal]="true" [maxFractionDigits]=" 2"></spiderly-number>
                    </div>
...
export class AgentBaseDetailsComponent {
    ...
    @Input() agentFormGroup: SpiderlyFormGroup<Agent>;
    ...
    agentSaveBodyName: string = nameof<AgentSaveBody>('agentDTO');
    ...
    @Input() showTemperatureForAgent = true;
    ...
    ngOnInit(){
        this.formGroup.initSaveBody = () => { 
            let saveBody = new AgentSaveBody();
            saveBody.agentDTO = this.agentFormGroup.getRawValue();
        ...
        this.formGroup.saveObservableMethod = this.apiService.saveAgent;
        this.formGroup.mainDTOName = this.agentSaveBodyName;
        ...
        this.route.params.subscribe(async (params) => {
            ...
            if(this.modelId > 0){
                forkJoin({
                    mainUIFormDTO: this.apiService.getAgentMainUIFormDTO(this.modelId),
                })
                .subscribe(({ mainUIFormDTO }) => {
                    this.initAgentFormGroup(new Agent(mainUIFormDTO.agentDTO));
                ...
                this.initAgentFormGroup(new Agent({id: 0}));
    ...
    initAgentFormGroup(agent: Agent) {
        this.baseFormService.addFormGroup<Agent>(
            this.agentFormGroup, 
            this.formGroup, 
            agent, 
            this.agentSaveBodyName,
            ['createdAt', 'modifiedAt']
        );
        this.agentFormGroup.mainDTOName = this.agentSaveBodyName;
            ...
            map(([currentUserPermissionCodes, isAuthorizedForSave]) => {
                if (currentUserPermissionCodes != null && isAuthorizedForSave != null) {
                    this.isAuthorizedForSave =
                        ...
                        (currentUserPermissionCodes.includes('InsertAgent') && this.modelId <= 0) || 
                        (currentUserPermissionCodes.includes('UpdateAgent') && this.modelId > 0) ||
                        isAuthorizedForSave;

                    if (this.isAuthorizedForSave) { 
                        this.agentFormGroup.controls.temperature.enable();

                    }
                    else{
                        this.agentFormGroup.controls.temperature.disable();
